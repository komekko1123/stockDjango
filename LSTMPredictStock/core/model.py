...
class Model():
	...
	def load_model(self, filepath): # load h5
		print('[Model] Loading model from file %s' % filepath)
		self.model = load_model(filepath)

	def build_model(self, configs): #用json當config 來build model
		timer = Timer()
		timer.start()
	...
	def train_generator(self, data_gen, epochs, batch_size, steps_per_epoch, save_dir,save_name): # use generator  但我的電腦這樣跑會有tensorflow的bug不推薦
		timer = Timer()
		timer.start()
		print('[Model] Training Started')
